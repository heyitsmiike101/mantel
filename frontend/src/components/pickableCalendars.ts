 ...
 *
 *  - It has to be writable. A calendar the provider says we can only read can't take events.
 *  - A *synced* calendar -- Google or Apple -- has to be syncing. The push queue skips
 *    calendars with sync_enabled off, so an event created on one would save here, appear
 *    on the wall, and never reach the provider — silently. Better not to offer it.
 *
 ...
 *  no toggle anywhere to fix it, since the Syncing switch is only shown for synced
 *  calendars.
 *
